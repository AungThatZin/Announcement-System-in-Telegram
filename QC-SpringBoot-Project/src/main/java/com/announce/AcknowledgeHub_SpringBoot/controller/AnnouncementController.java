...
//    }
    ...
    public ResponseEntity<List<Notification>> getUnseenNotifications(@PathVariable int userId) {
        List<Notification> notifications = notificationRepo.findByUserIdAndSeenFalse(userId);
        return ResponseEntity.ok(notifications);
    }

    @PutMapping("/mark-seen/{userId}")
    public ResponseEntity<?> markNotificationsAsSeen(@PathVariable int userId) {
        int updatedCount = notificationRepo.markNotificationsAsSeen(userId);
        if (updatedCount > 0) {
            return ResponseEntity.status(HttpStatus.OK)
                    .body("Marked " + updatedCount + " notifications as seen.");
        }
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body("No notifications to mark as seen.");
    }
